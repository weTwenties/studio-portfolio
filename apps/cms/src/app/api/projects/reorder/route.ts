...
export async function PATCH(request: Request) {
  try {
    ...
    if (!Array.isArray(body.ids) || body.ids.some((id) => typeof id !== "string" || !id.trim())) {
      return NextResponse.json({ message: "Invalid project order payload" }, { status: 400 });
    }
    ...
    return NextResponse.json({ message: "Failed to reorder projects", error: message }, { status: 500 });
  }
}
